    ...
    class Assistant:
        ...
        def __init__(self):
            ...
            self.cmds = {
                ...
                ('включи проводник', 'включи папку', 'мой компьютер'): self.exp,
                ('Кто твой создатель', 'Кто тебя создал'): self.azb,
            }
        ...
        def opener(self, task):
            links = {
                ...
                ('музыка', 'музыку', 'яндекс музыку', 'яндэкс музыку'): 'https://music.yandex.ru/',
        ...
        def azb(self):
            print('Авторское право вошло в чат)')
            webbrowser.open('https://vk.com/yamategesu', new=0)
            testaudio.creator()

    frame_layout = [[sg.Text("Нана-ваш голосовой помощник", size=(42, 3), font = ('any 15') )],
                    [sg.Text(
                        'Beta 4.3'+ c , size=(45, 2), font = ('any 15'))],
                    [GraphicButton('Запуск', '-NEXT-', green_pill64),
                        GraphicButton('Выйти', '-EXIT-', orange64)], ]

    layout = [[sg.Frame('NaNa', frame_layout, font=(
        'any 18'), background_color='black')]]

    window = sg.Window("Nana", layout, auto_size_buttons=False, default_button_element_size=(20,2), use_default_focus=False, finalize=True)
    # ---===--- The event loop --- #
    while True:
        ...
        if event in ('-EXIT-', None):  # Exit button or X
            break
        if event in ('-NEXT-', None):  # Exit button or Xa
            Assistant().cfile()
